...
usage() {
  cat <<'USAGE'
Usage: ingestion-status.sh --config <county-env-file> [--window-minutes 60]
...
for req in APPRAISAL_QUEUE_URL APPRAISAL_QUEUE_NAME APPRAISAL_FUNCTION_NAME PERMIT_QUEUE_URL PERMIT_QUEUE_NAME PERMIT_DLQ_URL PERMIT_FUNCTION_NAME PERMIT_JOB_PREFIX COUNTY_KEY; do
  if [[ -z "${!req:-}" ]]; then
...
PERMIT_EXTRACTED="$(${SCRIPT_DIR}/s3-prefix-count.sh --s3-uri "${PERMIT_JOB_PREFIX}/${COUNTY_KEY}/extracted/permits/" --window-minutes "$WINDOW_MINUTES")"
PERMIT_RAW_DETAILS="$(${SCRIPT_DIR}/s3-prefix-count.sh --s3-uri "${PERMIT_JOB_PREFIX}/${COUNTY_KEY}/raw/permit-details/" --window-minutes "$WINDOW_MINUTES")"
PERMIT_LISTS="$(${SCRIPT_DIR}/s3-prefix-count.sh --s3-uri "${PERMIT_JOB_PREFIX}/${COUNTY_KEY}/permit-lists/" --window-minutes "$WINDOW_MINUTES")"
