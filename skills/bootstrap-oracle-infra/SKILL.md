...
description: Verify and bootstrap the local pipeline stack required for county ingestion - Restate server, data directories, Postgres, and the elephant-pipeline services process registered with Restate. Use when starting county onboarding, when a run or registration fails because the stack is down or services are missing, or when setting up the pipeline on a fresh machine.
metadata:
  author: elephant-xyz
---
...
services process. Work happens in a checkout of `elephant-pipeline` with sibling repos
...
## Scaffold `elephant-pipeline` (if missing)

Layout:

```
elephant/
  elephant-pipeline/          # your durable pipeline project (scaffolded)
    docker-compose.yml
...
```
DATA_DIR=/absolute/path/to/elephant/elephant-pipeline/data  # absolute — sibling-repo commands rely on it
