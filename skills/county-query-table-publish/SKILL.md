...
4. **`DATABASE_URL`** points at the same query DB the load and consolidation export used
   (the local Postgres from `elephant-pipeline/.env` by default).
...
(`DATA_DIR` is the `elephant-pipeline` data dir, so staging lands at
...
Required env in `<publish-env>` (Filebase creds from `elephant-pipeline/.env` or a local
