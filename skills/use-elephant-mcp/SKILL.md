...
description: "Operating guide for exploring Elephant data through the elephant-mcp server: Oracle open-data properties and Overture business places, including appraisal, permits, Sunbiz, BBB, categories, geo filters, lexicon schemas, and verified transforms. Use for county property questions, business-category counts/groups, restaurants, nail salons, contractors, address mismatches, or schema fields — via MCP tools only. Not for direct Neon/IPFS access or county ingestion."
metadata:
  author: elephant-xyz
---
...
[`elephant-mcp`](https://github.com/elephant-xyz/elephant-mcp). The agent reads Oracle
open-data (IPFS), geo indexes, and lexicon schemas by calling MCP tools — never by shelling
out to IPFS, AWS, or ad-hoc HTTP.

Do **not** use this skill for:

- **Neon SQL** over ingested rows → `elephant-query-db` (`src/schema/*.ts` is the live schema)
- **County ingestion / refresh** → `oracle` agent + `onboard-county`

## Always read first

1. [`reference/mcp-setup.md`](./reference/mcp-setup.md) — MCP install, env vars, regenerating county maps from the catalog
...
- **Node.js 22.18+** (Node ≥26 requires the built `dist/index.js`; do not import `package.json` from raw TypeScript)
- **Elephant MCP** configured as server name **`elephant`**, installing **`main`** from
  `github:elephant-xyz/elephant-mcp#main` until npm publishes a build with `queryProperties`.
  Confirm **`elephant`** is enabled under the client's MCP settings.
- **Optional env** (not hard-coded in skill text):
  - `OPENAI_API_KEY` for `getVerifiedScriptExamples`, or AWS creds for Bedrock. Do **not** set
    an empty key — the server crashes on a blank `OPENAI_API_KEY`.
...
2. Call `getOracleDatasetInfo` on server **`elephant`** with the county under discussion
   ...
   [`reference/mcp-setup.md`](./reference/mcp-setup.md). Do not bypass with shell, IPFS CLI, or
   direct CID fetches.
...
1. **Property attribute / aggregate / count / filter** — `getPropertyQuerySchema` (learn the
   ...
   `county` defaults to `lee` and must match the MCP's `PROPERTY_QUERY_TABLE_MAP`. Coverage
   varies by county: Lee has no acreage/material (NULL); HOA (`hoa_flag`) is NULL everywhere —
   confirm with `getPropertyQuerySchema` / `SELECT count(col)` and say "not available for this
   county" rather than inventing.
...
8. **Missing permits** — if the county is in `PERMIT_QUERY_TABLE_MAP`, use
   `getPermitQuerySchema` → `queryPermits`. Otherwise `getPropertyPermits` with `parcelId`
   **and `countyFips`** (default `12071` = Lee). On-demand harvest needs pipeline ingress on
   the MCP; if that is not configured, report harvest unavailable instead of polling 90s.
...
- Hand off to `oracle` + `onboard-county` when the user wants to ingest or refresh source data.
