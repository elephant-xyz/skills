...
**OPEN IPFS parquet via MCP (NOT Neon)** — do not hand these questions off to Neon.
...
Only counties in `PERMIT_QUERY_TABLE_MAP` (currently Montgomery and Rock Island in the
example config). Do not use `getPropertyPermits` for county-wide permit counts.
...
| `sumPropertyValueInArea` | Sum of AVM in area | `county?`, exactly one of: `bbox`, `polygon` |
...
`PROPERTY_QUERY_TABLE_MAP`. Omitting `county` uses the default (Lee). `ORACLE_GEO_INDEX_IPNS`
is an optional fallback for counties not in the map — it is a Lee reference index, not a
substitute for passing `county`.
...
│   └─ getOracleProperty on hits (county)
...
- `getPropertyPermits` may return harvest-in-progress only when pipeline ingress is configured;
