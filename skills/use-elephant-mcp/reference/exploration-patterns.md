...
3. Else `getPropertyPermits` with `parcelId` **and `countyFips`** (Lee default `12071` is
   wrong for every other county). If harvest is not configured on the MCP, stop and say so.
4. If harvest-in-progress, wait ~90s and retry once
...
- Centroids only (not building footprints).
- Dataset may lag live county portals — cite `exportedAt` / `completedAt` from dataset info.
- For heavy analytics (joins, SQL, dashboards), hand off to `elephant-query-db` (live schema).
